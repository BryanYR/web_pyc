    ...
    <div class="pyc-container-section mx-auto grid md:grid-cols-2 gap-6">
      ...
      <div class="grid lg:grid-cols-2 gap-4">
        <div
          ...
          :key="item.key"
          class="flex flex-col items-center gap-4 p-4 hover:scale-105 duration-300 ease-in-out border border-primary-900 rounded-lg shadow-sm bg-white dark:bg-primary-900"
          :class="index === (infoPreventions.length - 1) ? 'lg:col-span-2' : ''"
        >
          <component :is="item.icon" class="w-12 h-12 md:w-16 md:h-16 text-primary-900 dark:text-white" />
