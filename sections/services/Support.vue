      ...
      <div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-4">
          <div
            ...
            :key="item.key"
            :class="['flex flex-col items-center gap-4 p-4 rounded-lg shadow-sm', (i % 2 === 0) ? 'bg-primary-700 text-white' : 'bg-[#f2f2f2]']"
          >
            <component :is="item.icon" :class="(i % 2 === 0) ? 'w-12 h-12 md:w-16 md:h-16 text-white' : 'w-12 h-12 md:w-16 md:h-16 text-primary-700'" />
            <div class="text-sm font-medium lg:text-base" :class="(i % 2 === 0) ? 'text-white' : 'text-gray-800'">{{ item.title }}</div>
          </div>
        </div>
        ...
        <!-- Second row: 5 cards -->
          <div class="grid grid-cols-1 sm:grid-cols-5 gap-4">
            <div
              ...
              :key="item.key"
              :class="['flex flex-col items-center gap-4 p-4 rounded-lg shadow-sm', ((i + 4) % 2 === 0) ? 'bg-primary-700 text-white' : 'bg-[#f2f2f2]']"
            >
              <component :is="item.icon" class="w-12 h-12 md:w-16 md:h-16" :class="((i + 4) % 2 === 0) ? ' text-white' : ' text-primary-700'" />
              <div class="text-sm font-medium lg:text-base" :class="((i + 4) % 2 === 0) ? ' text-white' : ' text-gray-800'">{{ item.title }}</div>
            </div>
          </div>
