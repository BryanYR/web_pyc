...
// Nuevo: archivo y previsualización para imagen (Banner principal)
const imageFile = ref<File | null>(null)
const imagePreview = ref<string>('')

// Generar/revocar URL temporal para previsualización local
...
const existingFileUrl = computed(() => {
  const raw =
    (props.initial?.fileBlog as unknown as string | null | undefined) || null
...
const previewAuthor = computed(() => ({
  name: author.value || 'PYC',
  avatar: '',
}))
const previewBreadcrumbs = computed(() => [
  { label: 'Blog', to: '/blog' },
  { label: title.value || 'Previsualización' },
])
const readTime = computed(() => estimateReadTime(content.value))
const previewKey = computed(
  () =>
    `${title.value}::${author.value}::${imageUrl.value}::${content.value.length}`
)
...
async function onSubmit() {
  // Si hay un archivo de imagen seleccionado, súbelo primero para obtener la URL
  if (imageFile.value) {
    try {
      ...
      if (res?.ok && res.data?.url) {
        ...
        console.error('Error subiendo imagen, respuesta inválida:', res)
      }
    } catch (e) {
...
<template>
  <div class="w-full">
      ...
      <div :class="showPreview ? 'lg:col-span-6' : 'lg:col-span-12'">
        <div class="grid grid-cols-1 md:grid-cols-12 gap-5">
          ...
          <div class="md:col-span-6 col-span-12">
            <label class="block text-sm font-medium mb-1" for="image-file"
              >Imagen (Banner principal)</label
            >
          ...
          <div class="col-span-6">
            <div v-if="imagePreview || imageUrl" class="mb-2">
              <img
                :src="imagePreview || imageUrl"
                alt="Previsualización"
                class="max-h-40 rounded border mx-auto"
              />
            </div>
          </div>
          <div class="col-span-6">
            <label for="current-file" class="block text-sm font-medium mb-1"
            ...
            <div v-if="existingFileUrl" class="text-sm">
              ...
              <p class="text-xs text-gray-500 mt-1">
                Si adjuntas un nuevo archivo, se reemplazará el existente.
              </p>
            </div>
    ...
    <div
      v-if="showPreview"
      class="fixed inset-0 z-50 bg-black/50 flex lg:hidden"
    >
